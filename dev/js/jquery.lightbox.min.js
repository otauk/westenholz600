...
(function(b) {
	b.fn.lightBox = function(a) {
		function q(d, c) {
			...
			} else for (var f = 0; f < c.length; f++) if ("gallery" != a.captionPosition) g = c[f].getAttribute("title"), a.imageArray.push([c[f].getAttribute("href"), g]);
			else if (1 < f) {
			...
			} else g = jQuery(c[f]).parent().next().html(), a.imageArray.push([jQuery(c[f]).parent().parent().children().children().attr("href"), g]);
		...
		function m() {
			...
			b("#jquery-lightbox").css({
				top: c[1] + (a.shrinkToFit ? a.shrinkPadding : d[3] / 10),
				left: c[0]
			})
		}
		function s(d, c) {
			...
			0 == e && 0 == h && (b.browser.msie ? p(250) : p(100));
